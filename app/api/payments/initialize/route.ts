...
export async function POST(request: Request) {
  try {
    ...
    // Final price already calculated when the owner
    // created or edited the property.
    const totalAmount = Number(property.display_price);
    ...
    const totalAmountPesewas = Math.round(
      totalAmount * 100
    );

    // The difference between the final student price
    // and the owner's accommodation price is the
    // payment processing amount.
    const paymentProcessingFee =
      totalAmount - amount;

    // STUVANA commission remains based on the
    // owner's accommodation price only.
    const commissionAmount =
      (amount * commissionRate) / 100;

    // Owner receives the accommodation price
    // minus STUVANA's commission.
    const ownerAmount =
      amount - commissionAmount;

    const reference =
      `STUVANA-${Date.now()}-${user.id.slice(0, 8)}`;
    ...
    const paystackResponse = await fetch(
      "https://api.paystack.co/transaction/initialize",
      {
        ...
        body: JSON.stringify({
          ...
          reference,
          callback_url:
            `${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/payment/callback`,
        }),
      }
    );
